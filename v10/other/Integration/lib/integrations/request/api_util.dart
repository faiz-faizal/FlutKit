...
class ApiUtil {
  ...
  static String GET_URL = 'http://api.openweathermap.org/data/2.5/weather?q=surat&appid={API_KEY}';
  static String POST_URL = 'http://api.openweathermap.org/data/2.5/weather?q=surat&appid={API_KEY}';
  ...
  static Future<http.Response> makeGetRequest() async {
    ...
    http.Response response = await http.get(GET_URL,headers: getHeader(requestType: RequestType.Get));
    return response;
  }

  static Future<http.Response> makePostRequest(Map<String,String> data) async {
    String body = json.encode(data);
    http.Response response = await http.post(GET_URL,headers: getHeader(requestType: RequestType.Get),body: body);
    return response;
  }
